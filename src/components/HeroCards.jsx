...
const HeroCards = () => {
  ...
  const updateCardsToShow = useCallback(() => {
    ...
    else if (width > 700) setCardsToShow(1);
...
useGSAP(() => {
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow- z-30">
      <button onClick={next} className="absolute z-50 right-[6%] 2xl:right-[26%] top-[30%] sm:top-[20%] transform -translate-y-1/2 flex items-center justify-center w-[44px] sm:w-[84px]  h-[42px] p-[1px] border border-[#38ff38] rounded-full">
        {/* Next Arrow SVG */}
        <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none">
          <path d="M35.3389 16.4108C36.2178 17.2897 36.2178 18.7171 35.3389 19.596L26.3389 28.596C25.46 29.4749 24.0326 29.4749 23.1537 28.596C22.2748 27.7171 22.2748 26.2897 23.1537 25.4108L28.3146 20.2499H2.24981C1.00527 20.2499 -0.00019455 19.2444 -0.00019455 17.9999C-0.00019455 16.7554 1.00527 15.7499 2.24981 15.7499H28.3146L23.1537 10.589C22.2748 9.71006 22.2748 8.28271 23.1537 7.40381C24.0326 6.5249 25.46 6.5249 26.3389 7.40381L35.3389 16.4038V16.4108Z" fill="white"/>
        </svg>
      </button>

      <button onClick={prev} className="absolute z-50 left-[6%] 2xl:left-[26%] top-[30%] sm:top-[20%] transform -translate-y-1/2 flex items-center justify-center w-[44px] sm:w-[84px] h-[42px] p-[1px] border border-[#38ff38] rounded-full">
        {/* Prev Arrow SVG */}
        <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none">
          <path d="M0.661133 16.4108C-0.217773 17.2897 -0.217773 18.7171 0.661133 19.596L9.66113 28.596C10.54 29.4749 11.9674 29.4749 12.8463 28.596C13.7252 27.7171 13.7252 26.2897 12.8463 25.4108L7.68535 20.2499H33.7502C34.9947 20.2499 36.0002 19.2444 36.0002 17.9999C36.0002 16.7554 34.9947 15.7499 33.7502 15.7499H7.68535L12.8463 10.589C13.7252 9.71006 13.7252 8.28271 12.8463 7.40381C11.9674 6.5249 10.54 6.5249 9.66113 7.40381L0.661133 16.4038V16.4108Z" fill="white"/>
        </svg>
      </button>

      <img src={leftblur} alt="" className="blur-img absolute -left-80 xl:-left-32 -top-18 z-30 2xl:h-[596px] opacity-70 2xl:opacity-95" />
      <img src={rightblur} alt="" className="blur-img absolute -right-80 xl:-right- -top-18 z-30 2xl:h-[596px] opacity-70 2xl:opacity-95" />
        ...
        >
          {allClasses.map((img, i) => {
            ...
            return (
              <div
                key={i}
                className="slide-card w-[881.19px] h-[240px] m:h-[495.66px] relative"
                style={{ width: `${100 / allClasses.length}%` }}
              >
                <img src={img} alt={`Slide ${i + 1}`} className="w-full h-full rounded-xl shadow-md object-cover" />
                <div
